    ...
    <div className="flex-1 flex flex-col items-center gap-5 md:gap-6 px-4 md:px-6 py-6 md:py-10 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 w-full max-w-md sm:max-w-none sm:w-auto">
        <button
          ...
          className="w-full sm:w-auto min-h-[44px] flex items-center justify-center gap-2 px-4 py-2.5 text-sm text-zinc-400 hover:text-zinc-200 border border-zinc-700 hover:border-zinc-600 rounded-lg transition-colors"
        >
        ...
        <a
          ...
          className="w-full sm:w-auto min-h-[44px] flex items-center justify-center gap-2 px-4 py-2.5 text-sm text-zinc-300 hover:text-white border border-zinc-700 hover:border-zinc-600 rounded-lg transition-colors"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
function ImageCard({ label, src, highlight }: { label: string; src: string; highlight?: boolean }) {
  return (
    <div className="space-y-2">
      <p className={['text-xs uppercase tracking-widest', highlight ? 'text-indigo-400' : 'text-zinc-500'].join(' ')}>
        {label}
      </p>
      <div
        className={[
          'relative rounded-xl overflow-hidden bg-zinc-900 shadow-2xl',
          highlight ? 'ring-1 ring-indigo-500/40' : 'ring-1 ring-zinc-800/60',
        ].join(' ')}
      >
